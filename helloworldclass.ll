; ModuleID = 'helloworldclass.cpp'
source_filename = "helloworldclass.cpp"
target datalayout = "e-m:o-i64:64-i128:128-n32:64-S128"
target triple = "arm64-apple-macosx13.0.0"

%class.Main = type { i8 }

@.str = private unnamed_addr constant [14 x i8] c"Hello world!\0A\00", align 1

; Function Attrs: noinline norecurse optnone ssp uwtable(sync)
define i32 @main() #0 {
  %1 = alloca i32, align 4
  %2 = alloca %class.Main, align 1
  store i32 0, i32* %1, align 4
  %3 = call %class.Main* @_ZN4MainC1Ev(%class.Main* noundef %2)
  ret i32 0
}

; Function Attrs: noinline optnone ssp uwtable(sync)
define linkonce_odr %class.Main* @_ZN4MainC1Ev(%class.Main* noundef returned %0) unnamed_addr #1 align 2 {
  %2 = alloca %class.Main*, align 8
  store %class.Main* %0, %class.Main** %2, align 8
  %3 = load %class.Main*, %class.Main** %2, align 8
  %4 = call %class.Main* @_ZN4MainC2Ev(%class.Main* noundef %3)
  ret %class.Main* %3
}

; Function Attrs: noinline optnone ssp uwtable(sync)
define linkonce_odr %class.Main* @_ZN4MainC2Ev(%class.Main* noundef returned %0) unnamed_addr #1 align 2 {
  %2 = alloca %class.Main*, align 8
  store %class.Main* %0, %class.Main** %2, align 8
  %3 = load %class.Main*, %class.Main** %2, align 8
  %4 = call i32 (i8*, ...) @printf(i8* noundef getelementptr inbounds ([14 x i8], [14 x i8]* @.str, i64 0, i64 0))
  ret %class.Main* %3
}

declare i32 @printf(i8* noundef, ...) #2

attributes #0 = { noinline norecurse optnone ssp uwtable(sync) "frame-pointer"="non-leaf" "min-legal-vector-width"="0" "no-trapping-math"="true" "probe-stack"="__chkstk_darwin" "stack-protector-buffer-size"="8" "target-cpu"="apple-m1" "target-features"="+aes,+crc,+crypto,+dotprod,+fp-armv8,+fp16fml,+fullfp16,+lse,+neon,+ras,+rcpc,+rdm,+sha2,+sha3,+sm4,+v8.5a,+zcm,+zcz" }
attributes #1 = { noinline optnone ssp uwtable(sync) "frame-pointer"="non-leaf" "min-legal-vector-width"="0" "no-trapping-math"="true" "probe-stack"="__chkstk_darwin" "stack-protector-buffer-size"="8" "target-cpu"="apple-m1" "target-features"="+aes,+crc,+crypto,+dotprod,+fp-armv8,+fp16fml,+fullfp16,+lse,+neon,+ras,+rcpc,+rdm,+sha2,+sha3,+sm4,+v8.5a,+zcm,+zcz" }
attributes #2 = { "frame-pointer"="non-leaf" "no-trapping-math"="true" "probe-stack"="__chkstk_darwin" "stack-protector-buffer-size"="8" "target-cpu"="apple-m1" "target-features"="+aes,+crc,+crypto,+dotprod,+fp-armv8,+fp16fml,+fullfp16,+lse,+neon,+ras,+rcpc,+rdm,+sha2,+sha3,+sm4,+v8.5a,+zcm,+zcz" }

!llvm.module.flags = !{!0, !1, !2, !3, !4, !5, !6, !7, !8}
!llvm.ident = !{!9}

!0 = !{i32 2, !"SDK Version", [2 x i32] [i32 13, i32 3]}
!1 = !{i32 1, !"wchar_size", i32 4}
!2 = !{i32 8, !"branch-target-enforcement", i32 0}
!3 = !{i32 8, !"sign-return-address", i32 0}
!4 = !{i32 8, !"sign-return-address-all", i32 0}
!5 = !{i32 8, !"sign-return-address-with-bkey", i32 0}
!6 = !{i32 7, !"PIC Level", i32 2}
!7 = !{i32 7, !"uwtable", i32 1}
!8 = !{i32 7, !"frame-pointer", i32 1}
!9 = !{!"Apple clang version 14.0.3 (clang-1403.0.22.14.1)"}
